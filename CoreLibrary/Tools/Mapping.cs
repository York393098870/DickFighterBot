...
public class Mapping
{
    ...
    {
        if (value >= maxRange) throw new ArgumentOutOfRangeException(nameof(value), "Value must be less than range.");

        if (value < 0) return -1.0 / (1.0 + Math.Exp(k * (value / maxRange + 0.5)));
    ...
    {
        if (value > maxRange) throw new ArgumentOutOfRangeException(nameof(value), "Value must be between 0 and M.");

        if (value < 0) return Math.Log(-value * k + 1) / Math.Log(maxRange * k + 1);
